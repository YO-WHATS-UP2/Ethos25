...
searchForm.addEventListener('submit', async function(event) {
    ...
    try {
        ...
        const timelineData = formatAndSortTimeline(result.activity); // Use your proven logic
...
// --- DATA PROCESSING (Adapted from your original script) ---

/**
 * Processes the raw activity object from the server, formats it, 
 * and sorts it chronologically into a clean array for rendering.
 * @param {object} activityData - The 'activity' object from the server response.
 * @returns {Array<object>} A sorted array of timeline events.
 */
function formatAndSortTimeline(activityData) {
    const timeline = [];

    // Helper function to extract and format events from a single data source
    const extractEvents = (label, events, primaryField, secondaryField = null) => {
        ...
        events.forEach(event => {
            ...
            if (isNaN(sortTime.getTime())) return; // Skip any entries with invalid dates

            // Create a structured object for each timeline event
            let item = {
                type: label,
                title: label,
                time: timeStart,
                details: '',
                sortTime: sortTime // Use this for accurate sorting
            };

            // Customize details based on the event type
            switch (label) {
                case 'Card Swipe':
                    item.details = `Location: ${event.location_id || 'Unknown'}`;
                    break;
                case 'Lab Booking':
                    const timeEnd = event[secondaryField];
                    item.time = `${timeStart} - ${timeEnd}`;
                    item.details = `Booked: ${event.room_id || 'Unknown Lab'}`;
                    item.attended = event['attended (YES/NO)'] === 'YES';
                    break;
                case 'Wi-Fi Log':
                    item.details = `Connected to AP: ${event.ap_id || 'Unknown'}`;
                    break;
                case 'CCTV Frame':
                    item.details = `Detected near: ${event.location_id || 'Unknown'}`;
                    break;
                case 'Library Checkout':
                    item.details = `Checked out book: ${event.book_id || 'Unknown'}`;
                    break;
                case 'Note':
                    // *** CHANGE HERE: Use the 'text' field from the note data ***
                    item.details = `Note (${event.category || 'General'}): ${event.text || 'No details'}`;
                    break;
            }
            timeline.push(item);
        });
    ...
    // Process each activity type from the server data
    ...
    // Sort the entire timeline chronologically
 ...
 * @param {object} profile - The profile object from the API response.
 */
 ...
 * @param {Array<object>} timelineData - The sorted array of timeline events.
 */
function renderTimeline(timelineData) {
    timelineList.innerHTML = '';
    if (timelineData.length === 0) {
        timelineList.innerHTML = '<p class="event-details">No activity found for this entity.</p>';
        return;
    }
 ...
 * @param {object} item - A single processed timeline event.
 * @returns {string} The HTML for the timeline item.
 */
function createTimelineItemHTML(item) {
    ...
    // If it's a lab booking, add the attendance status
    ...
    // Assemble the final HTML for the item
    return `
        <div class="timeline-item">
            <div class="timeline-icon-wrapper ${iconDetails.color}">
                ${iconDetails.svg}
            </div>
 ...
 * @param {string} type - The event type (e.g., 'Card Swipe').
 * @returns {object} An object with color and svg properties.
 */
function getIconDetails(type) {
    ...
    return icons[type] || icons['Card Swipe']; // Default to the card swipe icon
}
